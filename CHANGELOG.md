...
- Several smaller correctness fixes from the audit: **PR diffs** (`src/diff.ts`) now record the *new* path for a renamed/copied file — `git diff --name-status` emits `R<score>\toldpath\tnewpath`, and the old code joined the fields into a single `oldpath\tnewpath` string that corrupted the "Files Modified" listing and silently dropped renamed files from later per-file scans. **Markdown→HTML** (`src/formatter.ts`) now escapes the contents of inline code spans, so common type signatures like `` `Promise<void>` `` no longer emit raw `<void>` markup that browsers swallow (fenced blocks were already escaped — this restores the symmetry). The main command's **`--render-diagrams`** flag no longer renders on *every* run: because the option carries a default value, the old `!== undefined` presence check was always true; it now detects real CLI presence via the option source. And **watch mode** (`src/watch.ts`) advances its commit cursor only after a successful re-analysis, so a transient failure during regeneration is retried on the next poll instead of leaving the generated docs permanently stale.
