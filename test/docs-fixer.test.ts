...
import { updateVersionNumbers, addMissingFrameworks, updateCLIUsage, fixDocumentation } from "../src/docs-fixer.js";
import type { DocsAnalysisResult, VersionMismatch, FrameworkIssue, CLIDrift } from "../src/docs-analyzer.js";
  ...
  describe("addMissingFrameworks", () => {
    ...
    it("should return no changes when no README exists", async () => {
      const issues: FrameworkIssue[] = [
        { framework: "react", status: "missing", version: "18.0" }
      ];
    ...
    it("should insert before Features section", async () => {
      await writeFile(join(tempDir, "README.md"), "# My Project\n\n## Features\n\n- Cool stuff\n", "utf-8");
      const issues: FrameworkIssue[] = [
        { framework: "angular", status: "missing" }
      ];
      const result = await addMissingFrameworks(tempDir, issues);
      expect(result.success).toBe(true);
    });
